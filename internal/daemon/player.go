...
func (p *Player) loadAudioFile(filePath string) error {
	...
	case ".aiff", ".aif":
		streamer, format, err := decodeAIFF(file)
		if err != nil {
			file.Close()
			return fmt.Errorf("failed to decode AIFF: %w", err)
		}
		p.streamer = streamer
		p.format = format
		p.file = file
	...
	default:
		file.Close()
		return fmt.Errorf("unsupported audio format: %s (supported: .mp3, .wav, .aiff, .aif)", ext)
	}
